...
public sealed class BookingEmailNotificationService
{
    ...
    public async Task SendBookingApprovedToCustomerEmailAsync(
        User customer,
        Booking booking,
        Schedule schedule,
        string? providerFirstName)
    {
        var subject = $"Booking Confirmed: {schedule.Title}";
        var htmlBody = GetApprovedCustomerEmailTemplate(
            customer.FirstName,
            schedule.Title,
            schedule.StartTime,
            schedule.EndTime,
        ...
        var textBody = $"Your booking for \"{schedule.Title}\" on {schedule.StartTime:MMM dd, yyyy 'at' h:mm tt}{withProvider} has been approved!";

        await SendEmailAsync(customer.Email, subject, htmlBody, textBody);
    }
...
<body style=""font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;"">
    ...
    private static string GetApprovedCustomerEmailTemplate(
        string customerFirstName,
        string scheduleTitle,
        DateTime startTime,
        DateTime endTime,
        ...
        string? notes)
    {
        ...
        var providerRow = string.IsNullOrEmpty(providerFirstName)
            ? ""
            : $@"<tr style=""background: #f9fafb;""><td style=""padding: 8px 12px; color: #6b7280;"">Provider</td><td style=""padding: 8px 12px;"">{System.Net.WebUtility.HtmlEncode(providerFirstName)}</td></tr>";
...
<body style=""font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;"">
    <div style=""max-width: 600px; margin: 0 auto; padding: 20px;"">
        ...
        <div style=""background-color: #f9fafb; padding: 30px; border-radius: 0 0 8px 8px;"">
            <p>Hi {System.Net.WebUtility.HtmlEncode(customerFirstName)},</p>
            ...
            <p style=""color: #6b7280; font-size: 14px;"">You can view your booking details from your dashboard.</p>
            <p style=""margin-top: 20px;"">Best regards,<br>BookMeApp Team</p>
        </div>
    </div>
</body>
